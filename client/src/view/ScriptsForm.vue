...
export default {
    data() {
        return {
            script_name: 'copy_list',  // 默认的 script_name
            formFields: [],
            formData: {},
            logContent: '',  // 初始化日志内容
            isConnected: false,
    ...
    methods: {
        async loadFormConfig() {
            try {
                ...
                if (viewFormConfig) {
                    ...
                    throw new Error(`未找到 script_name 为 ${this.script_name} 的表单配置`);
                }
            } catch (error) {
        ...
        async saveFormData() {
            ...
            await db.put('formDataStore', formDataCopy, this.script_name);
        },
        ...
        async loadFormData() {
            ...
            const storedData = await db.get('formDataStore', this.script_name);
            if (storedData) {
